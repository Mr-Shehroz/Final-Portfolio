...
const Services = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      // === 1. Header: Staggered blur-to-sharp with float ===
      ...
      // === 2. Cards: Staggered "pop-in" with rotation & blur ===
      ...
      // === 3. Hero Image: Elastic entrance with blur ===
  ...
  // === Sparkle on card hover (unchanged) ===
  useEffect(() => {
    if (hoveredIndex !== null && cardRefs.current[hoveredIndex]) {
      const sparkle = cardRefs.current[hoveredIndex].querySelector('.services-card-sparkle');
      if (sparkle) {
        gsap.fromTo(
          sparkle,
  ...
  // === Sparkle on image hover (unchanged) ===
  useEffect(() => {
    if (imgContainerRef.current) {
  ...
  return (
    <>
      ...
      <section
        id="services"
        className="text-white py-14 sm:py-20 px-5 mx-auto max-w-[1386px]"
        ref={sectionRef}
      >
        {/* Header */}
        <div ref={headerRef} className="text-center mb-10 sm:mb-12">
          ...
          <p className="mt-4 text-gray-300 max-w-3xl mx-auto leading-relaxed text-sm sm:text-base">
            I deliver end-to-end technical solutions that empower businesses to operate smarter, scale
            faster, and innovate continuously — combining code with intelligence.
          </p>
        </div>

        <div className="flex flex-col-reverse md:flex-row gap-8 md:gap-12 items-center justify-between w-full">
          {/* Left Column - Service Cards */}
          <div className="w-full md:w-[51%] space-y-5 sm:space-y-8 shrink-0">
